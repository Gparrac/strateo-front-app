    ...
    async deleteItems(data) {
      ...
      const response =
        this.selectedItems.length == 1
          ? await measureApi.delete(`?measure_id=${this.selectedItems[0].id}`)
          : await measureApi.delete(
              `?${this.selectedItems.map((element) => `measure_ids[]=${element.id}&`).join("")}`
            );
