...
<script>
import PurchaseOrderApi from "@/services/Forms/PurchaseOrderApi";
...
export default {
  ...
  data: () => ({
    ...
    loading: false,
    //delete items
    keyQueryDelete: "purchaseOrder_id",
    mainKeyDelete: ["date"],
    secondKeyDelete: ["supplier","fullname"],
    selectedItems: [],
    toggleDelete: false,
    //optional data
  ...
  methods: {
    async loadItems(
      ...
      const response = await purchaseOrderApi.read(params.toString());
    ...
    async deleteItems(data) {
      ...
      const response =
        this.selectedItems.length == 1
          ? await purchaseOrderApi.delete(`?purchaseOrder_id=${this.selectedItems[0].id}`)
          : await purchaseOrderApi.delete(
              `?${this.selectedItems.map((element) => `purchaseOrder_ids[]=${element.id}&`).join("")}`
            );
